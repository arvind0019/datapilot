...
export const CommandPalette: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/80 backdrop-blur-none pt-[12vh] animate-in fade-in duration-100 p-4 font-mono text-xs">
      <div 
        className="w-full max-w-2xl overflow-hidden rounded-md border-[3px] border-black bg-[#161b22] shadow-[10px_10px_0px_#000000]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Search Input Bar */}
        <div className="p-3 border-b-[3px] border-black bg-[#0d1117]">
          <div className="brutal-box flex items-center px-3.5 py-2.5 bg-[#161b22]">
            <Search className="h-4 w-4 text-[#00f0ff] mr-3 flex-shrink-0" />
            <input
              ref={inputRef}
              type="text"
              value={query}
              ...
              placeholder="Type a command or search entities (e.g. 'Postgres', 'MRR', 'Deploy')..."
              className="w-full bg-transparent font-mono text-xs text-white placeholder-slate-400 outline-none font-bold"
            />
            <kbd 
              onClick={() => setIsCommandPaletteOpen(false)}
              className="cursor-pointer brutal-badge bg-white text-black text-[9px]"
            >
              ESC
            </kbd>
          </div>
        ...
        {/* Results List */}
        <div className="max-h-96 overflow-y-auto p-2 space-y-1.5 bg-[#161b22]">
          {filteredItems.length === 0 ? (
            <div className="py-12 text-center font-mono text-xs text-slate-400 font-bold">
            ...
            filteredItems.map((item, idx) => {
              ...
              return (
                <div
                  ...
                  className={`flex cursor-pointer items-center justify-between rounded px-3 py-2 transition-all ${
                    isSelected
                      ? 'bg-[#ffee00] text-black border-2 border-black shadow-[4px_4px_0px_#000000] translate-x-[-1px]'
                      : 'text-slate-200 hover:bg-[#21262d] border border-transparent'
                  }`}
                >
                  <div className="flex items-center space-x-3 truncate">
                    <div className={`flex h-8 w-8 items-center justify-center rounded border-2 border-black shadow-[2px_2px_0px_#000] ${
                  ...
                  <ArrowRight className={`h-4 w-4 ${isSelected ? 'text-black' : 'text-slate-500'}`} />
                </div>
              );
            })
          )}
        ...
        {/* Footer shortcuts */}
        <div className="flex items-center justify-between border-t-[3px] border-black bg-[#0d1117] px-4 py-2 text-[10px] text-slate-300 font-bold">
          ...
          <span className="text-[#ffee00]">DATAPILOT NEO-BRUTALIST COMMAND MESH</span>
        </div>
      </div>
    </div>
  );
