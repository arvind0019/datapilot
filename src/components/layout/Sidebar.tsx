...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside 
      className={`relative z-20 flex flex-col border-r-[3px] border-black bg-[#161b22] transition-all duration-150 ease-out select-none ${
        isSidebarCollapsed ? 'w-[76px]' : 'w-64'
      }`}
    >
      {/* Brand Header */}
      <div className="flex h-16 items-center justify-between border-b-[3px] border-black px-4 bg-[#0d1117]">
        {!isSidebarCollapsed ? (
          ...
          <div className="mx-auto flex h-9 w-9 items-center justify-center rounded-md bg-[#ffee00] text-black border-2 border-black shadow-[3px_3px_0px_#000000]">
            <svg className="h-5 w-5 fill-black" viewBox="0 0 24 24">
              <polygon points="12 2 2 7 12 12 22 7 12 2" />
            </svg>
          </div>
      ...
      {/* Workspace Switcher */}
      {!isSidebarCollapsed && (
        <div className="border-b-[3px] border-black p-3 bg-[#0d1117]">
      ...
      <div className="flex-1 overflow-y-auto px-2.5 py-3 space-y-1.5">
        <div className="px-2 pb-1 text-[9px] font-mono font-black uppercase tracking-widest text-slate-400">
          {!isSidebarCollapsed ? '// PLATFORM WORKSPACES' : '///'}
        </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`group flex w-full items-center rounded-md px-3 py-2 text-xs font-black transition-all duration-100 cursor-pointer ${
                isActive 
              ...
              } ${isSidebarCollapsed ? 'justify-center px-1.5' : 'justify-between'}`}
              title={isSidebarCollapsed ? item.label : undefined}
            >
              <div className="flex items-center space-x-2.5 truncate">
                <Icon className={`h-4 w-4 flex-shrink-0 ${isActive ? 'text-black' : 'text-slate-400 group-hover:text-white'}`} />
                {!isSidebarCollapsed && <span className="truncate uppercase font-mono tracking-tight">{item.label}</span>}
              </div>
      ...
      <div className="p-3 border-t-[3px] border-black bg-[#0d1117]">
        {!isSidebarCollapsed ? (
          <div className="brutal-panel p-3 bg-[#ffee00] text-black border-2 border-black shadow-[4px_4px_0px_#000] space-y-2">
            ...
            <button
              onClick={() => openCopilotWithPrompt('Perform a full workspace diagnostic check and highlight optimization opportunities.')}
              className="w-full brutal-btn bg-black text-white hover:bg-slate-900 py-1.5 text-[11px] font-black"
            >
              AUDIT MESH
            </button>
          </div>
      ...
      {/* Bottom Status */}
      {!isSidebarCollapsed && (
        <div className="border-t-[3px] border-black px-3.5 py-2 text-[10px] font-mono font-black flex items-center justify-between bg-black text-white">
          <div className="flex items-center space-x-1.5">
